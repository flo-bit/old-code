...
var particleCount = 2000;   // max number of particles
                            // [right/left arrow] add/subtract 50

var numberOfSlices = 30;     // number of slices in noise space (along z-axis)
                            // [c/v] change number of slices

var visibility = 130;    // [,/.] alpha value of particles
var deletionSpeed = 25; // [g/h] alpha value of black background drawn every
...
const sliceDistance = 0.02;    // distance between noise slices
const maxspeed = 3;     // maxspeed of particles, independent of framerate 
                        // (so higher framerate will make them go faster)

const hueMultiplier = 20.0;    // particles will multiply noise result 
                                // by this to get hue
const accMultiplier =  Math.PI * 2;
/////////
...
function draw() {
    if(moveParticles) {
        ...
        if (particles.length < particleCount) {
            addParticles(particleCount - particles.length);
...
function calculateForce(x,y,z) {
    //let m = sin(x*0.1) * cos(y * .2134) * abs(x) / abs(y);
    if((x*y)%2 == 0) return 0; 
    else return calculateForce(x/2,y*3,z) + .1;
    
    let m = x / width * y / height + y / width;//x / y * sin(y * 0.1) + noise(x * scl, y * scl, z * scl);
        
        //sin(frameCount * 0.01) * 0.5 + .5;//sin(x * .1) * PI / 2;
    m = cos(m);
    return min(1,max(0,m));
    
    //noise(x*scl+xMove,y*scl+yMove,z+zMove);
}
